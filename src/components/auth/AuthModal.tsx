...
import { X, Check, AlertCircle, Gamepad2, Upload, Image as ImageIcon } from "lucide-react";
...
export default function AuthModal({
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setError("La imagen no debe superar los 2MB de tamaño.");
      return;
    }
          ...
          <form onSubmit={handleRegister} className="p-6 space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-300 mb-2">
                Elige tu Avatar Gamer (o sube tu dibujo/foto)
              </label>
              ...
              <div className="flex items-center gap-2.5 mb-3 overflow-x-auto pb-1">
                {PRESET_AVATARS.map((av, idx) => (
                  <button
                    ...
                    className={`relative h-12 w-12 rounded-xl overflow-hidden border-2 transition-all shrink-0 bg-[#121620] cursor-pointer flex flex-col items-center justify-center p-1 ${
                      selectedAvatar === av.url && !customAvatarUrl
                        ? "border-white scale-105 shadow-lg bg-white/[0.08]"
